...
<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
  ...
  <script>
    ...
    $(document).ready(function(){
		MIN_NUM = 1;
		MAX_NUM = $(opener.document).find('#maxCnt' + '${sel_room_num}').val();
    	var str = "";
		for(var i = MIN_NUM; i <= MAX_NUM ; i++){
			str += "<tr id=" + i +" >";
			str += "<td id='no_"+i+"'>"+ i +"</td>";
			str += "<td id='dept_"+i+"'></td>";
			str += "<td id='name_"+i+"'></td>";
			str += "<td id='rank_"+i+"'></td>";
			str += "<td id='memId_"+i+"'></td>";
			str += "<td id='order_"+i+"'></td>";
			str += "<td id='del_"+i+"'></td>";
			str += "</tr>";
		}
		console.log(str);
    ...
    $('#tt').tree({
	 ...
	 function apply(){
		 ...
		 for(var i = MIN_NUM; i <= MAX_NUM ; i++){
			if($('#memId_'+i).text() != ""){
				str += "<div id='" + $('#memId_'+i).text() +"' class='resChecked'>";
				str += $('#name_'+i).text() + $('#rank_'+i).text();
				str += "</div>";
		 	}
		 }
